...
(define decode (λ (input text-len segments)
  ...
  (car (for/fold ([acc : (Pair String Code-Point) (cons "" input)]) ([_ (in-range text-len)])
    ...
    (match-define (list char lbound ubound) (decode-lookup-char bound segment-list))
    (cons (~a decoded char) (abs (/ (- bound lbound) (- ubound lbound))))))))

(: decode-lookup-char (-> Code-Point (Listof (Pair Char Segment-Bounds)) (List Char Code-Point Code-Point)))
